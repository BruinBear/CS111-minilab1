...
copy_stack(process_t *dest, process_t *src)
{
	...
	dest_stack_bottom = dest_stack_top - (src_stack_top - src_stack_bottom);/* YOUR CODE HERE set bottom based on offset*/;
...
schedule(void)
{
	...
	while (1) 
	{
		...
		if (proc_array[pid].p_state == P_RUNNABLE)
			run(&proc_array[pid]);
			//if blocked check wait process
		else if(proc_array[pid].p_state == P_BLOCKED)
		{
			pid_t wait = proc_array[pid].p_wait_pid;
			if(proc_array[wait].p_state ==P_ZOMBIE)
			{
				proc_array[pid].p_registers.reg_eax = proc_array[wait].p_exit_status;
				//this process is back to runnable
				proc_array[pid].p_state = P_RUNNABLE;
				proc_array[pid].p_wait_pid = -1;
				run(&proc_array[pid]);
			}
